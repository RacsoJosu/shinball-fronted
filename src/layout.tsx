    ...
    <div className="flex w-full">
      ...
      <div className=" flex flex-row-reverse h-10">
        ...
        <div className="flex flex-col items-center justify-center w-full gap-2 my-4">
          <PropiedadesIcon
            className={
              ` fill-sky-900   ${isCollapsed ? " h-10 w-10": "h-25 w-25"}   mx-auto`
            }
          />
          {!isCollapsed && (
        ...
        <ul className="flex flex-col gap-1  items-center w-full justify-center">
          {routes.map(({ Icon, name, to }) => (
            <li className="w-full" key={to}>
              ...
              >
                {Icon ? (
                  <Icon className=" text-center w-6 h-6   " />
                ) : null}

                {!isCollapsed && (

                  <span className={`text-lg max-sm:hidden  ${isCollapsed ? "opacity-0" : "opacity-100"} transition-opacity duration-500 delay-200`}>{name}</span>
          )}
              </NavLink>
            </li>
          ))}
